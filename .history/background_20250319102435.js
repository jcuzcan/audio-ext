...
let audioChunks = [];
let workletNodeAvailable = false; // Track the availability of workletNode
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.command === "startRecording") {
    ...
    sendResponse({ status: "Error", message: message.message });
    ...
    workletNodeAvailable = message.workletNodeAvailable;
    sendResponse({ status: "Recording started", workletNodeAvailable: workletNodeAvailable });
  }
});
...
function startRecording(sendResponse) {
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    ...
    // Send message to content.js to start recording
